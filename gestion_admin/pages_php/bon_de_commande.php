  ...
  <section class="section">
    ...
    <div class="section-body">
      ...
      if (isset($_POST['save'])) {
        ...
        $req = $fun->all_in_one("INSERT INTO `d_bc`(`client`, `mf`, `tel`, `adresse`, `date`) VALUES ('$client', '$mf', '$tel', '$adresse', '$date')");
        $id_client = $fun->get_one('SELECT id FROM d_bc ORDER BY id DESC LIMIT 1');
        if (!isset($_POST['count'])) {
          $_POST['count'] = 1;
        }
        for ($i = 1; $i <= $_POST['count']; $i++) {
          $fun->all_in_one('INSERT INTO d_ligne_bc(id_bc, designation, prix, qte) VALUES ("' . $id_client['id'] . '" ,"' . $_POST['desg_' . $i] . '","' . $_POST['prix_' . $i] . '", "' . $_POST['qte_' . $i] . '")');
        }
        if ($req) {
          $msg_cl = 1;
        }
      ...
      if (isset($_POST['supprimer_element'])) {
        $dell = $fun->all_in_one('DELETE FROM d_bc WHERE id = ' . $_POST['supprimer_element']);
        $dell_ligne = $fun->all_in_one('DELETE FROM d_ligne_bc WHERE id_bc = ' . $_POST['supprimer_element']);
      ...
      <p class="section-lead"> Liste des Bons des Commandes </p>
            ...
            <div class="card-body">
              <div class="table-responsive">
                <table class="table table-striped" id="table-1">
                  ...
                  <tbody>
                    <?php $bc = $fun->get_list('SELECT * FROM d_bc');
                    foreach ($bc as $key => $list) { ?>
                      <tr>
                        ...
                        <td class="text-center" style="padding: 0.75rem;">
                          ...
                          <a class="btn btn-sm btn-primary text-white" target="_blank" href="Imprimer_bc?id=<?= $list['id']; ?>"><i class="fa fa-print"></i> / <i class="fa fa-eye"></i></a>
                        </td>
                      </tr>
                    <?php } ?>
                  </tbody>
          ...
          <div class="form-group row">
            ...
            <div class="col-md-6" id="show_select">
              <select name="mf" class="form-control select2" style="width: 100%; height: 15px !important">
                <?php
                $d_bc = $fun->get_list('SELECT * FROM d_bc');
                foreach ($d_bc as $ls) {
                ?>
                  <option value="<?= $ls["mf"]; ?>">
                    <?= sprintf('%s', $ls["mf"]); ?>
