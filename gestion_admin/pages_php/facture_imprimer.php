<?php
  ...
  $facture = $fun->get_one('SELECT * FROM d_facture WHERE id = '.$_GET['id']);
?>
<head>
  <meta charset="UTF-8">
        ...
        <div class="document-container p-3" id="section-to-print">
          ...
          <div class="row mt-5">
            <div class="col-md-4 offset-8">
              <b>Client</b> : <?=$facture['client'];?><br>
              <b>Mobile</b> : <?=$facture['tel'];?><br>
              <b>Adresse</b> : <?=$facture['adresse'];?><br>
              <b>MF</b> : <?=$facture['mf'];?><br>
            </div>
          </div>
          <hr>
          <div class="row">
            <div class="col-md-12 text-right">
              <h5>Sousse, le <?=date_format(date_create($facture['date']),"d/m/Y");?></h5><br>
            </div>
            <div class="col-md-12">
              <h3> Facture N°: <?='F-'.sprintf('%06d', $facture['id']);?></h3>
            </div>
          </div>
          ...
          <div class="document-table mt-4">
            <table class="table table-bordered" width="100%">
              ...
              <tbody>
                <?php
                  $t = 0;
                  $ligne = $fun->get_list('SELECT * FROM d_ligne_facture WHERE id_facture = '.$_GET['id']);
