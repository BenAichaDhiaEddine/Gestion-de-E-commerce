  ...
  <section class="section">
    ...
    <div class="section-body">
      ...
      if (isset($_POST['save'])) {
        ...
        $req = $fun->all_in_one("INSERT INTO `d_devis`(`client`, `mf`, `tel`, `adresse`, `date`) VALUES ('$client', '$mf', '$tel', '$adresse', '$date')");
        $id_client = $fun->get_one('SELECT id FROM d_devis ORDER BY id DESC LIMIT 1');
        if (!isset($_POST['count'])) {
          $_POST['count'] = 1;
        }
        for ($i = 1; $i <= $_POST['count']; $i++) {
          $fun->all_in_one('INSERT INTO d_ligne_devis(id_devis, designation, prix, qte) VALUES ("' . $id_client['id'] . '" ,"' . $_POST['desg_' . $i] . '","' . $_POST['prix_' . $i] . '", "' . $_POST['qte_' . $i] . '")');
        }
        if ($req) {
          $msg_cl = 1;
        }
      ...
      if (isset($_POST['supprimer_element'])) {
        $dell = $fun->all_in_one('DELETE FROM d_devis WHERE id = ' . $_POST['supprimer_element']);
        $dell_ligne = $fun->all_in_one('DELETE FROM d_ligne_devis WHERE id_devis = ' . $_POST['supprimer_element']);
      ...
      <p class="section-lead"> Liste des devis </p>
            ...
            <div class="card-body">
              <div class="table-responsive">
                <table class="table table-striped" id="table-1">
                  ...
                  <tbody>
                    <?php $Devis = $fun->get_list('SELECT * FROM d_devis');
                    foreach ($Devis as $key => $list) { ?>
                      <tr>
                        ...
                        <td class="text-center" style="padding: 0.75rem;">
                          ...
                          <a class="btn btn-sm btn-primary text-white" target="_blank" href="Imprimer_devis?id=<?= $list['id']; ?>"><i class="fa fa-print"></i> / <i class="fa fa-eye"></i></a>
                        </td>
                      </tr>
                    <?php } ?>
                  </tbody>
  ...
  <div class="modal-dialog modal-lg" role="document">
    ...
    <form class="modal-content" method="POST" enctype="multipart/form-data">
      ...
      <div class="modal-body section">
          ...
          <div class="form-group row">
            ...
            <div class="col-md-6" id="show_select">
              <select name="mf" class="form-control select2" style="width: 100%; height: 15px !important">
                <?php
                $d_devis = $fun->get_list('SELECT * FROM d_devis');
        ...
        <h2 class="section-title">Détail Devis</h2>
...
if (isset($msg_cl) and ($msg_cl == 1)) {
  echo "
      <script>
        new Toast({
          message: 'Devis à été créé avec <strong>succès</strong>',
          type: 'success'
        });
      </script>
    ";
