  ...
  <section class="section">
    ...
    <div class="section-body">
      ...
      if (isset($_POST['save'])) {
        ...
        $id_exist = $fun->get_count('SELECT * FROM d_fournisseur WHERE email = "' . $email . '"');

        if (!$id_exist) {
          $req = $fun->all_in_one("INSERT INTO `d_fournisseur`(`raison`, `email`, `logo`, `tel`, `date_ajout`) VALUES ('$raison', '$email', '$img', '$tel', '$date_ajout')");
          if ($req) {
            if (!empty($_FILES['img']['name'])) {
              move_uploaded_file($_FILES['img']['tmp_name'], '../assets/img/img_four/' . $img);
            }
            $msg_cl = 1;
          }
        } else {
      ...
      if (isset($_POST['update-btn'])) {
        ...
        if (isset($_POST["raison"]) and (!empty($_POST["raison"]))) {
          $sql = 'UPDATE d_fournisseur SET raison="' . $_POST['raison'] . '",date_ajout = "' . $date_ajout . '"  WHERE id="' . $_POST["update-btn"] . '"';
          $fun->update($sql);
        }
        if (isset($_POST["email"]) and (!empty($_POST["email"]))) {
          $sql = 'UPDATE d_fournisseur SET email="' . $_POST['email'] . '"' . '",date_ajout = "' . $date_ajout . '"  WHERE id="' . $_POST["update-btn"] . '"';
          $fun->update($sql);
        }
        if (isset($_POST["tel"])) {
          $sql = 'UPDATE d_fournisseur SET tel="' . $_POST['tel'] . '",date_ajout = "' . $date_ajout . '"  WHERE id="' . $_POST["update-btn"] . '"';
          $fun->update($sql);
        }
        if (!empty($_FILES['img']['name'])) {
          $img = time() . '__' . $_FILES['img']['name'];
          $sql = 'UPDATE d_fournisseur SET logo="' . $_POST['img'] . '",date_ajout = "' . $date_ajout . '"  WHERE id="' . $_POST["update-btn"] . '"';
          $fun->update($sql);
          move_uploaded_file($_FILES['img']['tmp_name'], '../assets/img/img_four/' . $img);
        }
        $msg_update = 1;
      }
      //if post supprimer fournisseur 
      if (isset($_POST['supprimer_element'])) {
        $id_c = $fun->get_one('SELECT * FROM d_fournisseur WHERE id = ' . $_POST['supprimer_element']);
        $req = $fun->all_in_one('DELETE FROM `d_fournisseur` WHERE id = ' . $_POST['supprimer_element']);
        if ($req) {
          if ($id_c['logo'] != 'p-250.png') {
            unlink('../assets/img/img_four/' . $id_c['logo']);
          }
          $msg_del = 1;
        }
      }
      ...
      <p class="section-lead"> Liste des fournisseurs </p>
      <div class="row">
        <div class="col-12">
            ...
            <div class="card-body">
              <div class="table-responsive">
                <table class="table table-striped" id="table-1">
                  ...
                  <tbody>
                    <?php $d_c = $fun->get_list('SELECT * FROM d_fournisseur');
                    foreach ($d_c as $ls_c) {
                    ?>
                      <tr>
                        <td><?= $ls_c['raison']; ?></td>
                        <td><img src="../assets/img/img_four/<?= $ls_c['logo']; ?>" class="rounded-circle" width="35"></td>
                        <td><?= $ls_c['email']; ?></td>
                        <td><?= $ls_c['tel']; ?></td>
...
<!--modal UPDATE -->
<?php
$first = $fun->get_one('SELECT id FROM `d_fournisseur` ORDER BY id ASC LIMIT 1');
$last = $fun->get_one('SELECT id FROM `d_fournisseur` ORDER BY id DESC LIMIT 1');
for ($i = $first['id']; $i <= $last['id']; $i++) : $user = $fun->get_one('SELECT * FROM d_fournisseur WHERE id =' . $i); ?>
        ...
        <div class="modal-body" id="update">
          ...
          <div class="form-group">
            ...
            <?php if ($user["logo"] == "p-250.png") {
              $img_user = '../assets/img/img_four/p-250.png';
            } else {
              $img_user = '../assets/img/img_four/' . $user['logo'];
            }
            ?>
            <br>
...
<script>
  function _details_(id) {
    $.ajax({
      url: 'ajax/fournisseur_detail.php',
      type: 'POST',
      dataType: "html",
      data: {
        id: id
